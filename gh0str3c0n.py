...
def naabu():
    with log.progress(f'Running Naabu....') as p:
       with ThreadPoolExecutor(max_workers=1) as executor:
            p.status(colored("Running.",'yellow', attrs=['bold']))
...
def m4skup():
    with log.progress(f'Running M4skup....') as p:
       with ThreadPoolExecutor(max_workers=1) as executor:
            p.status(colored("Running.",'yellow', attrs=['bold']))
...
def gau():
    with log.progress(f'Running Get All Urls....') as p:
       with ThreadPoolExecutor(max_workers=1) as executor:
            p.status(colored("Running.",'yellow', attrs=['bold']))
...
def waybackurls():
    with log.progress(f'Running Waybackurls....') as p:
       with ThreadPoolExecutor(max_workers=1) as executor:
            p.status(colored("Running.",'yellow', attrs=['bold']))
...
def dirsearch():
    with log.progress(f'Running Dirsearch....') as p:
       with ThreadPoolExecutor(max_workers=1) as executor:
            p.status(colored("Running.",'yellow', attrs=['bold']))
...
def amass():
    with log.progress(f'Running Amass....') as p:
       with ThreadPoolExecutor(max_workers=1) as executor:
            p.status(colored("Running.",'yellow', attrs=['bold']))
...
def paramspi():
    with log.progress(f'Running ParamSpider....') as p:
       with ThreadPoolExecutor(max_workers=1) as executor:
            p.status(colored("Running.",'yellow', attrs=['bold']))
...
def gf():
    with log.progress(f'Running Pattern Matching for (XSS, SQLI, SSRF, SSTI, IDOR, LFI, RCE, and Open Redirect)....') as p:
